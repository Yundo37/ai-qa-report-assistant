    ...
    <section className="min-w-0 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {issues.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-4 overflow-hidden rounded-2xl border border-indigo-100 bg-indigo-50/30 p-2">
            <div className="grid grid-cols-[88px_minmax(0,1fr)_74px_70px_70px] px-2 pb-2 text-[11px] font-semibold text-slate-400">
              <span>Key</span>
              <span>Summary</span>
              <span>Priority</span>
              <span>Status</span>
            ...
            {representativeIssues.map((issue, index) => (
              <div
                key={`${issue.key}-${index}`}
                className="grid grid-cols-[88px_minmax(0,1fr)_74px_70px_70px] items-center gap-2 rounded-xl bg-white px-2.5 py-2 text-xs shadow-sm shadow-indigo-50/60 [&+&]:mt-1.5"
              >
