...
function createRcFlowLabel(rcProgress: RcProgressSummary) {
  ...
  if (rcLabels.length >= 2) {
    return `Jira RC 흐름: ${rcLabels[0]}~${rcLabels[rcLabels.length - 1]}`;
  }
    ...
    <section className="flex h-full min-w-0 flex-col rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {cumulativeRows.length > 0 ? (
        <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200">
          <div className="grid grid-cols-[44px_48px_48px_72px_minmax(0,1fr)] bg-slate-50 px-2.5 py-2 text-center text-[10px] font-semibold text-slate-500">
            <span>RC</span>
            <span>신규</span>
            <span>처리</span>
            <span>누적 잔여</span>
          ...
          {cumulativeRows.map(
            ({
              ...
              <div
                key={item.rc}
                className="grid grid-cols-[44px_48px_48px_72px_minmax(0,1fr)] items-center border-t border-slate-100 px-2.5 py-2.5 text-center text-xs"
              >
